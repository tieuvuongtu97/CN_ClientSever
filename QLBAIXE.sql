...
--UPDATE DBO.KHACHHANG SET HOTEN = N'VŨ TRUNG' WHERE ID = N'KH0002'

--TRIGGER CHECK BIỂN SỐ CÓ TRÙNG KHÔNG
DROP TRIGGER TG_CHECK_BIENSO
CREATE TRIGGER TG_CHECK_BIENSO ON VE_THANG
FOR INSERT, UPDATE
AS
BEGIN
...
BEGIN
	BEGIN TRAN
		SAVE TRAN ST1
			...
			SET HOTEN = @HOTEN, SDT = @SDT, DIACHI = @DIACHI, SO_CCCD = @SO_CCCD, AVT = @AVT
			WHERE ID = @ID
		SAVE TRAN ST2
			UPDATE DBO.VE_THANG
...
--EXEC PR_EDIT_KH N'KH0004', N'VŨ TRUNG', N'0988111111', N'HÀ NỘI', N'012345678999', N'D:\BienvaEm\em1.jpg', 200000, N'SUZUKI', N'30N1-55555'
--insert dbo.VE_THANG(ID,SODU,KHACHHANG_ID) values('',0,null)


---- PROC XÓA KHÁCH HÀNG
CREATE PROC PR_DEL_KH(
	@ID VARCHAR(6)
)
AS
BEGIN
	BEGIN TRAN
		...
		SAVE TRAN SAVETRAN2
			UPDATE DBO.VE_THANG SET TRANGTHAI = 0 WHERE KHACHHANG_ID = @ID
		IF @@ERROR <>0
			BEGIN
...
SELECT * FROM KHACHHANG
SELECT * FROM VE_THANG
DELETE KHACHHANG WHERE ID = 'KH0012'
DECLARE @COUT INT = 0
WHILE(@COUT <20)
	BEGIN
		 ...
		 SET  @COUT= @COUT +1
	END
EXEC PR_DKKH_MOI N'VŨ NGŨ', N'0905302123', N'BẮC KAN', N'1964209961', NULL, N'VT0015', 100000, N'YAMAHA-SIRIUS-ĐEN', N'95B2-27235'
